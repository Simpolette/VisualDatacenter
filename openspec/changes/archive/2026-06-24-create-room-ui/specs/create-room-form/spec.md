...
The form SHALL contain fields matching the backend `CreateRoomDTO`: `name` (text, required), `location` (text, optional), `widthM` (number, required, positive), `depthM` (number, required, positive), `heightM` (number, optional). The frontend SHALL validate these fields against a defined Zod schema before submission.

#### Scenario: Required fields are enforced
- **WHEN** the user submits the form with `name` blank or `widthM`/`depthM` empty or non-positive
...
- **WHEN** the user leaves `location` and `heightM` blank and fills all required fields
- **THEN** the form submits successfully with `location` as null and `heightM` as null
